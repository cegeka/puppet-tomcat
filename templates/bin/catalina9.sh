...
# Computing specific variable expansion required for systemd

if [ -f /etc/sysconfig/<%= @tomcat_instance_name %> ]; then
	. /etc/sysconfig/<%= @tomcat_instance_name %>
fi
...
if [ "$1" = "debug" ] ; then
  ...
  if [ "$1" = "-security" ] ; then
    ...
    eval $_NOHUP "\"$_RUNJAVA\"" "\"$CATALINA_LOGGING_CONFIG\"" $LOGGING_MANAGER "$JAVA_OPTS" "$CATALINA_OPTS" \
      ...
      -Djava.io.tmpdir="\"$CATALINA_TMPDIR\"" \
      org.apache.catalina.startup.Bootstrap "$@" start \
      2>&1 | /usr/sbin/httplog -s $CATALINA_BASE/logs/catalina.log $CATALINA_BASE/logs/catalina.%Y-%m-%d.log &
